...
included_directories=("packages/components" "packages/ember-flight-icons" "packages/flight-icons/catalog.json")
...
for file_to_check in $files_to_check; do
	...
	if [ "$file_is_included" = "true" ]; then
		trigger_ci=true
		echo "Component package (or related) files changed - triggered ci: $trigger_ci"
		echo "trigger-ci=$trigger_ci" >>"$GITHUB_OUTPUT"
		exit 0
	fi
done
