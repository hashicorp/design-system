...
import MockAppMainGenericFilterBar from './filter-bar/index';

import type { Filters } from './filter-bar/index';

export interface MockAppMainGenericSuperSelectFilterSignature {
...
export default class MockAppMainGenericSuperSelectFilter extends Component<MockAppMainGenericSuperSelectFilterSignature> {
  ...
  valuesFromFilter = (filters: Filters, name: string) => {
    const filter = filters[name];
    if (!filter) return;

    if (Array.isArray(filter)) {
      if (filter.length === 1) return filter[0]?.value;
      return filter.map((f: Filter[]) => f.value);
    }
    return filter.value;
  };
  ...
  get demoModelFilteredData() {
    const filterItem = (item: Filters): boolean => {
