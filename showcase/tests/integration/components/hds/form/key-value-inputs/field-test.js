...
module(
  ...
  function (hooks) {
    ...
    test('it should render as required with `@isRequired` argument', async function (assert) {
      await render(hbs`
        <Hds::Form::KeyValueInputs::Field id="test-form-key-value-field" @isRequired={{true}} @rowIndex={{0}} as |F|>
          <F.Label>Label</F.Label>
        </Hds::Form::KeyValueInputs::Field>
      `);
    ...
    test('it should render as optional with `@isOptional` argument', async function (assert) {
      await render(hbs`
        <Hds::Form::KeyValueInputs::Field id="test-form-key-value-field" @isOptional={{true}} @rowIndex={{0}} as |F|>
          <F.Label>Label</F.Label>
        </Hds::Form::KeyValueInputs::Field>
      `);
    ...
    test('it should call `@onInsert/@onRemove` callbacks when added/removed', async function (assert) {
      ...
      await render(
        hbs`
          {{#if this.isRendered}}
            <Hds::Form::KeyValueInputs::Field @onInsert={{this.onInsert}} @onRemove={{this.onRemove}} />
          {{/if}}
        `,
      );
