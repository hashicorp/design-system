...
module(
  'Integration | Component | hds/form/key-value-pair/field',
  function (hooks) {
    setupRenderingTest(hooks);

    test('it should render the component with a CSS class that matches the component name', async function (assert) {
      await render(hbs`<Hds::Form::KeyValuePair::Field id="test-form-key-value-field" as |F|>
      ...
      assert
        ...
        .hasClass('hds-form-key-value-pair__field');
    });

    test('it should render the subcomponents with the correct CSS classes', async function (assert) {
      await render(hbs`<Hds::Form::KeyValuePair::Field id="test-form-key-value-field" @rowIndex={{0}} as |F|>
      <F.Label>Label</F.Label>
      <F.TextInput @value="Value" />
      <F.HelperText>Helper text</F.HelperText>
      <F.Error>Error text</F.Error>
      ...
      assert
        .dom('#test-form-key-value-field .hds-form-key-value-pair__field-label')
        .hasText('Label row 1');
      assert.dom('#test-form-key-value-field .hds-form-text-input').exists();
      assert
        .dom(
          '#test-form-key-value-field .hds-form-key-value-pair__field-helper-text',
        )
        .hasText('Helper text');
      assert
        .dom('#test-form-key-value-field .hds-form-key-value-pair__field-error')
        .hasText('Error text');
    });
    ...
    test('it should render as required with `@isRequired` argument', async function (assert) {
      await render(hbs`<Hds::Form::KeyValuePair::Field id="test-form-key-value-field" @isRequired={{true}} @rowIndex={{0}} as |F|>
      <F.Label>Label</F.Label>
      <F.TextInput @value="Value" />
      <F.HelperText>Helper text</F.HelperText>
      <F.Error>Error text</F.Error>
    ...
    test('it should render as optional with `@isOptional` argument', async function (assert) {
      await render(hbs`<Hds::Form::KeyValuePair::Field id="test-form-key-value-field" @isOptional={{true}} @rowIndex={{0}} as |F|>
      <F.Label>Label</F.Label>
      <F.TextInput @value="Value" />
      <F.HelperText>Helper text</F.HelperText>
      <F.Error>Error text</F.Error>
    ...
    test('it should render as invalid if `@isInvalid` is true', async function (assert) {
      await render(hbs`<Hds::Form::KeyValuePair::Field id="test-form-key-value-field" @isInvalid={{true}} @rowIndex={{0}} as |F|>
      <F.Label>Label</F.Label>
      <F.TextInput @value="Value" />
      <F.HelperText>Helper text</F.HelperText>
      <F.Error>Error text</F.Error>
    ...
    test('it should add `data-width` if `@width` is provided', async function (assert) {
      await render(
        hbs`<Hds::Form::KeyValuePair::Field id="test-form-key-value-field" @width="50%" />`,
      );
      assert
        .dom('#test-form-key-value-field')
    ...
    <F.HelperText>Helper text</F.HelperText>
      ...
      const errorId = document.querySelector(
        '#test-form-key-value-field .hds-form-key-value-pair__field-error',
      ).id;
