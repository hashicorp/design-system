...
module(
  ...
  function (hooks) {
    ...
    test('it should render the component with a CSS class that matches the component name', async function (assert) {
      await render(<template><Field @onChange={{NOOP}} /></template>);
    ...
    test('it should render the correct CSS class if @isInvalid is true', async function (assert) {
      await render(
        <template><Field @onChange={{NOOP}} @isInvalid={{true}} /></template>,
      );
      assert
    ...
    test('it should render the trigger with a custom id', async function (assert) {
      await render(
        <template><Field @id="my-super-select" @onChange={{NOOP}} /></template>,
      );
      assert
    ...
    test('it automatically provides all the ID relations between the elements', async function (assert) {
      await render(
        <template>
          <Field @extraAriaDescribedBy="extra" @onChange={{NOOP}} as |F|>
            <F.Label>This is the label</F.Label>
            <F.HelperText>This is the helper text</F.HelperText>
            <F.Error>This is the error</F.Error>
          </Field>
      ...
      let control = find('.ember-basic-dropdown-trigger');
    ...
    test('it should append an indicator to the label text when user input is required', async function (assert) {
      await render(
        <template>
          <Field @isRequired={{true}} @onChange={{NOOP}} as |F|>
            <F.Label>This is the label</F.Label>
          </Field>
        </template>,
      );
    ...
    test('it should append an indicator to the label text when user input is optional', async function (assert) {
      await render(
        <template>
          <Field @isOptional={{true}} @onChange={{NOOP}} as |F|>
            <F.Label>This is the label</F.Label>
          </Field>
        </template>,
      );
