...
module(
  ...
  function (hooks) {
    ...
    test('it should allow @isOpen to override an internal _isOpen=true', async function (assert) {
      const context = tracked<Record<'isOpen', boolean | undefined>>({
        isOpen: undefined,
      });
    ...
    test('it should allow the internal _isOpen to override @isOpen=true', async function (assert) {
      await render(
        <template>
          <DisclosurePrimitive @isOpen={{true}} id="test-disclosure-primitive">
            <:toggle as |t|>
              <button
                type="button"
                id="test-toggle-button"
    ...
    test('it should allow the internal _isOpen to override @isOpen=false', async function (assert) {
      await render(
        <template>
          <DisclosurePrimitive @isOpen={{false}} id="test-disclosure-primitive">
            <:toggle as |t|>
              <button
                type="button"
                id="test-toggle-button"
