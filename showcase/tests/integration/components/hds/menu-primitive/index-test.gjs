...
import { click, triggerEvent, triggerKeyEvent, render, resetOnerror } from '@ember/test-helpers';
import MenuPrimitive from "@hashicorp/design-system-components/components/hds/menu-primitive/index";
...
module('Integration | Component | hds/menu-primitive/index', function (hooks) {
  ...
  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(<template><MenuPrimitive id="test-menu-primitive" /></template>);
  ...
  test('it should hide the "content" when the "toggle" is deactivated via "Escape"', async function (assert) {
    await render(<template>
      <MenuPrimitive id="test-menu-primitive">
        <:toggle as |t|>
          <button type="button" id="test-menu-primitive-button" {{on "click" t.onClickToggle}} />
        </:toggle>
        <:content>
