...
module(
  ...
  function (hooks) {
    ...
    test('it should render the component with a CSS class that matches the component name', async function (assert) {
      await render(
        <template>
          <Button @text="toggle text" id="test-toggle-button" />
        </template>,
      );
    ...
    test('it should render the chevron "down" by default', async function (assert) {
      await render(
        <template>
          <Button @text="text toggle" id="test-toggle-button" />
        </template>,
      );
    ...
    test('it should render a badge if @badge is defined', async function (assert) {
      await render(
        <template>
          <Button @text="Lorem ipsum" @badge="badge" id="test-toggle-button" />
        </template>,
      );
      assert
    ...
    test('it should render a badge count if @count is defined', async function (assert) {
      await render(
        <template>
          <Button @text="Lorem ipsum" @count="3" id="test-toggle-button" />
        </template>,
      );
      assert
    ...
    test('it should render the correct CSS size class if the @size prop is declared', async function (assert) {
      await render(
        <template>
          <Button @text="text toggle" @size="small" id="test-toggle-button" />
        </template>,
      );
      assert
        .dom('#test-toggle-button')
    ...
    test('it should throw an assertion if an incorrect value for @color is provided', async function (assert) {
      ...
      await render(
        <template>
          {{! @glint-expect-error - assertion testing invalid value }}
          <Button @text="text toggle" @color="foo" id="test-toggle-button" />
        </template>,
      );
      assert.throws(function () {
        throw new Error(errorMessage);
    ...
    test('it should throw an assertion if an incorrect value for @size is provided', async function (assert) {
      ...
      await render(
        <template>
          {{! @glint-expect-error - assertion testing invalid value }}
          <Button @text="text toggle" @size="foo" />
        </template>,
      );
      assert.throws(function () {
        throw new Error(errorMessage);
