...
module('Integration | Component | hds/dropdown/index', function (hooks) {
  ...
  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(
      <template>
        <HdsDropdown id="test-dropdown" as |D|>
    ...
    await render(
      <template>
        <HdsDropdown id="test-dropdown" as |D|>
          ...
          <D.Interactive
            @route="page-components.dropdown"
            id="test-list-item-interactive"
          >
            interactive
          </D.Interactive>
    ...
    await render(
      <template>
        <HdsDropdown id="test-dropdown" as |D|>
          <D.ToggleButton @text="toggle button" id="test-toggle-button" />
          <D.Header id="test-header">Header</D.Header>
          <D.Interactive @route="page-components.dropdown">
            interactive
          </D.Interactive>
          <D.Footer id="test-footer">Footer</D.Footer>
        </HdsDropdown>
  ...
  test('it renders the content sub-components even when closed, when `@preserveContentInDom` is `true`', async function (assert) {
    await render(
      <template>
        <HdsDropdown id="test-dropdown" @preserveContentInDom={{true}} as |D|>
          <D.ToggleButton @text="toggle button" id="test-toggle-button" />
          <D.Header id="test-header">Header</D.Header>
          <D.Interactive id="test-list-item" @route="page-components.dropdown">
  ...
  test('it should render the content aligned on the left if the value of @listPosition is "bottom-left"', async function (assert) {
    await render(
      <template>
        <HdsDropdown id="test-dropdown" @listPosition="bottom-left" as |D|>
          <D.ToggleButton @text="toggle button" id="test-toggle-button" />
          <D.Interactive @route="page-components.dropdown">
            interactive
          </D.Interactive>
  ...
  test('it should render the element as `inline` if the value of @isInline is "true"', async function (assert) {
    await render(
      <template>
        <HdsDropdown id="test-dropdown" @isInline={{true}} as |D|>
          <D.ToggleButton @text="toggle button" id="test-toggle-button" />
          <D.Interactive @route="page-components.dropdown">
            interactive
          </D.Interactive>
  ...
  test('it should render the content with a fixed width if a @width value is passed', async function (assert) {
    await render(
      <template>
        <HdsDropdown @width="248px" id="test-dropdown" as |D|>
          <D.ToggleButton @text="toggle button" id="test-toggle-button" />
          <D.Interactive @route="page-components.dropdown">
            interactive
          </D.Interactive>
  ...
  test('it should render the content with the same width as the ToggleButton if @matchToggleWidth is set', async function (assert) {
    await render(
      <template>
        <HdsDropdown id="test-dropdown" @matchToggleWidth={{true}} as |D|>
          <D.ToggleButton
            {{style width="200px"}}
            @text="toggle button"
  ...
  test('it should hide the content when an interactive element triggers `close`', async function (assert) {
    await render(
      <template>
        <HdsDropdown id="test-dropdown" as |D|>
          <D.ToggleButton @text="toggle button" id="test-toggle-button" />
          <D.Interactive id="test-list-item-interactive" {{on "click" D.close}}>
            interactive
          </D.Interactive>
        </HdsDropdown>
      </template>,
  ...
  test('it should invoke the `onFocusOut` callback', async function (assert) {
    ...
    // callback when the focusout event has no `relatedTarget` (i.e. focus
    // moved to nowhere). Dispatch such an event explicitly so the test is
    // not coupled to browser-specific implicit refocus behavior.
    await triggerEvent('#test-dropdown', 'focusout', { relatedTarget: null });
