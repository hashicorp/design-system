...
import CopyItem from "@hashicorp/design-system-components/components/hds/dropdown/list-item/copy-item";
...
module(
  ...
  function (hooks) {
    ...
    test('it should render the component as a <li> element with a CSS class that matches the component name', async function (assert) {
      await render(
        <template><CopyItem @text="copy-item" id="test-list-item-copy-item" /></template>,
      );
    ...
    test('it should throw an assertion if @text is missing/has no value', async function (assert) {
      ...
      await render(<template><CopyItem /></template>);
