...
import { render, click, resetOnerror, settled, triggerKeyEvent } from '@ember/test-helpers';
...
module('Integration | Component | hds/side-nav/index', function (hooks) {
  ...
  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(
      <template><SideNav id="test-side-nav" @hasA11yRefocus={{false}} /></template>,
    );
  ...
  test('it renders the `a11y-refocus` elements with the right properties provided as arguments', async function (assert) {
    await render(<template>
      <SideNav @a11yRefocusSkipTo="test-skip-to" @a11yRefocusSkipText="test-skip-text" @a11yRefocusNavigationText="test-navigation-text" />
    </template>);
    assert
  ...
  test('it is not "responsive" if `isResponsive` is false', async function (assert) {
    await render(
      <template><SideNav id="test-side-nav" @isResponsive={{false}} /></template>,
    );
    assert
      .dom('#test-side-nav')
  ...
  test('if the @ariaLabel argument is not provided the default aria-labelledby value is set on the toggle button', async function (assert) {
    await render(
      <template><SideNav @isCollapsible={{true}} id="test-side-nav" /></template>,
    );
    assert
  ...
  test('if the @ariaLabel argument is provided the label value is set on the toggle button', async function (assert) {
    await render(
      <template><SideNav @isCollapsible={{true}} id="test-side-nav" @ariaLabel="Test" /></template>,
    );
    assert
      .dom('.hds-side-nav__toggle-button')
  ...
  test('when the viewport changes from desktop to mobile, it automatically collapses and becomes inert', async function (assert) {
    ...
    await render(<template>
      <SideNav @isCollapsible={{true}} @onDesktopViewportChange={{this.onDesktopViewportChange}}>
        <:header as |H|>
          <span id="test-side-nav-header" data-test-minimized={{H.isMinimized}} />
        </:header>
        <:body as |B|>
  ...
  test('it should call `onToggleMinimizedStatus` function if provided', async function (assert) {
    ...
    await render(
      <template><SideNav @isCollapsible={{true}} @onToggleMinimizedStatus={{this.onToggleMinimizedStatus}} /></template>,
    );
    await click('.hds-side-nav__toggle-button');
    assert.ok(toggled);
  });
