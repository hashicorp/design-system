...
module('Integration | Component | hds/side-nav/portal', function (hooks) {
  ...
  test('it should render the component with a CSS class that matches the component name', async function (assert) {
    await render(
      <template>
        <HdsSideNavPortalTarget id="test-side-nav-portal-target" />
      </template>,
    );
    assert
  ...
  test('we can use custom a custom name for the target portal', async function (assert) {
    await render(
      <template>
        <HdsSideNavPortalTarget @targetName="test-side-nav-portal-target" />
        <HdsSideNavPortal @targetName="test-side-nav-portal-target">
          <div id="test-side-nav-content-portaled" />
        </HdsSideNavPortal>
      </template>,
    );
  ...
  test('it renders the panel items provided via portal, in the right DOM location', async function (assert) {
    await render(
      <template>
        <HdsSideNavPortalTarget id="test-side-nav-content" />
        <HdsSideNavPortal as |Nav|>
          <Nav.BackLink
            @text="Back-link"
            @href="#"
            id="test-side-nav-list-backlink"
          />
  ...
  test('it should render with the correct aria-label attribute passed down to the "list" parent', async function (assert) {
    await render(
      <template>
        <HdsSideNavPortalTarget />
        <HdsSideNavPortal @ariaLabel="test">
          <div />
        </HdsSideNavPortal>
      </template>,
    );
  ...
  test('it marks inactive subnavs', async function (assert) {
    await render(
      <template>
        <HdsSideNavPortalTarget />
        <HdsSideNavPortal data-test-side-nav-panel as |Nav|>
      ...
      <template>
        ...
        <HdsSideNavPortal as |Nav|>
          <Nav.Link
            ...
            @href="#"
            id="test-side-nav-list-link-1"
            data-test-side-nav-link-2
          />
        </HdsSideNavPortal>
        <HdsSideNavPortal as |Nav|>
          <Nav.Link
            @text="Some other link content"
            @href="#"
            id="test-side-nav-list-link-2"
            data-test-side-nav-link-2
          />
        </HdsSideNavPortal>
        <HdsSideNavPortal as |Nav|>
          <Nav.Link
            @text="The last link content"
            @href="#"
            id="test-side-nav-list-link-3"
            data-test-side-nav-link-3
          />
        </HdsSideNavPortal>
      </template>,
    );
