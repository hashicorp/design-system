...
export const buildImplementFigmaFramePromptMessages = (
  ...
  const workflowText = [
    ...
    '4. For each unmatched node, surface it explicitly in your output as a human-review item; do not invent a component for it.',
    '5. Emit the final template using HDS components, design tokens for any custom styling, and `hds-<component-name>` BEM class names if you add wrappers. Do not hard-code colors, spacing, or typography values that should come from tokens.',
    '',
