...
// TEMP - sort all the files in the dist folder (so it's easy to compare the output)

console.log('\n\n------------------\nEXTRA - Sorting files content alphabetically...\n\n');
...
async function sortDistFolderContent() {

  console.log(`\nCleaning up the "sorted" dist folder`);
  const sortedDistFolder = distFolder + "-sorted";
  fs.emptyDirSync(sortedDistFolder);
  ...
  try {
    ...
    for (const filePath of allFiles) {
      ...
      const sortedFilePath = filePath.replace("/dist/", "/dist-sorted/");
      await fs.ensureDir(path.dirname(sortedFilePath));
      await fs.writeFile(sortedFilePath, sortedContent);
      console.log(`Sorted content of file: ${path.relative(sortedDistFolder, sortedFilePath)}`);
    }
  } catch (error) {
    console.error('Error sorting files content:', error);
  }
}

await sortDistFolderContent();
