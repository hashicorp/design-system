...
export function generateElevationHelpers(tokensElevation: TransformedTokens, tokensSurface: TransformedTokens, outputCssVars: boolean): Helpers {
  ...
  Object.keys(tokensElevation).forEach((key: string) => {
    ...
    if (levelValues && Object.prototype.hasOwnProperty.call(levelValues, 'box-shadow')) {
      ...
      const value = outputCssVars ? `var(--hds-elevation-${levelName}-box-shadow)` : levelValues['box-shadow'].$value;
  ...
  Object.keys(tokensSurface).forEach((key: string) => {
    ...
    if (levelValues && Object.prototype.hasOwnProperty.call(levelValues, 'box-shadow')) {
      ...
      const value = outputCssVars ? `var(--hds-surface-${levelName}-box-shadow)` : levelValues['box-shadow'].$value;
