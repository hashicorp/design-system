...
export default class HdsPopoverPrimitiveComponent extends Component<HdsPopoverPrimitiveSignature> {
  ...
  setupPrimitiveContainer = modifier(
    ...
    // @ts-expect-error - the `eager` option is not defined in the `ember-modifier` types (and not documented too, see: https://github.com/ember-modifier/ember-modifier/issues/774)
    { eager: true }
  );
  ...
  onFocusOut(event: FocusEvent) {
    ...
    if (this.containerElement) {
      ...
      if (xxx && !this.containerElement.contains(xxx)) {
        this.hidePopover();
      }
    }
  }
