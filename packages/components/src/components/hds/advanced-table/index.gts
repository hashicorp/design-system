...
import HdsAdvancedTableBody from './body.gts';
...
> extends Component<HdsAdvancedTableSignature<T>> {
  ...
  get hasRowsWithChildren(): boolean {
    ...
    return model.some((record) => {
      const children = (record as Record<string, unknown>)[this.childrenKey] as
  ...
  get sortCriteria(): string | HdsAdvancedTableSortingFunction<unknown> {
    ...
    const currentColumn = columns.find(
      (column) => column.key === this.currentSortBy
    );
                      ...
                      (hash
                        Tr=(this.asTr
                          (component
                            ...
                            depth=row.depth
                            isCompositeItemDisabled=(not row.isVisible)
                            ...
                            isSelectable=(if
                              isSelectionAllowed this.isSelectable
                            )
