...
import type { HdsAdvancedTableTdSignature } from './td.gts';
...
export default class HdsAdvancedTable extends Component<HdsAdvancedTableSignature> {
  ...
  get gridTemplateColumns(): string {
    ...
    if (!this.columnWidths) {
      ...
      for (let i = 0; i < this.columnWidths.length; i++) {
        style += ` ${
          this.columnWidths[i] ? this.columnWidths[i] : DEFAULT_COLUMN_WIDTH
        }`;
      }
    }
        ...
        >
          <HdsAdvancedTableTr
            ...
            @sortBySelectedOrder={{if
              (eq this._sortBy @selectableColumnKey)
              this._sortOrder
            }}
            ...
            {{#each @columns as |column index|}}
              {{#if column.isSortable}}
                <HdsAdvancedTableThSort
                  ...
                  @isStickyColumn={{if
                    (and (eq index 0) @hasStickyFirstColumn)
                    true
                  }}
                  @isStickyColumnPinned={{this.isStickyColumnPinned}}
                >
  ...
  -------------------------------------------------------------------------------------------- }}
          ...
          {{#each (sortBy this.getSortCriteria this._tableModel.rows) key=this.identityKey as |record index|}}
            {{#if this._tableModel.hasRowsWithChildren}}
