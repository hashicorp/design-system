...
export interface HdsAccordionSignature {
  ...
  Blocks: {
     ...
     * Consumers can use it like: <Accordion as |Acc|><Acc.Item>...</Acc.Item></Accordion>
     */
    default: [
      {
...
/**
 * HdsAccordion Component
 *
 * DATA FLOW ARCHITECTURE:
 * ┌─────────────────────────────────────────────────────────────────┐
 * │ Consumer Template                                               │
 * │   <Hds::Accordion @size="large" @type="flush" as |Acc|>        │
 * │     <Acc.Item @title="Section 1">Content here</Acc.Item>       │
 * │   </Hds::Accordion>                                            │
 * └─────────────────────────────────────────────────────────────────┘
 *                              │
 *                              ▼
 * ┌─────────────────────────────────────────────────────────────────┐
 * │ HdsAccordion Component (this file)                              │
 * │   - Validates @size and @type arguments                        │
 * │   - Computes CSS class names for styling hooks                 │
 * │   - Yields contextual Item component with inherited props      │
 * └─────────────────────────────────────────────────────────────────┘
 *                              │
 *                              ▼
 * ┌─────────────────────────────────────────────────────────────────┐
 * │ <cds-accordion> Web Component                                   │
 * │   - Container for accordion items                              │
 * │   - Manages accordion group behavior                           │
 * └─────────────────────────────────────────────────────────────────┘
 *                              │
 *                              ▼
 * ┌─────────────────────────────────────────────────────────────────┐
 * │ HdsAccordionItem (via Acc.Item contextual component)            │
 * │   - Receives inherited size, type, forceState                  │
 * │   - Wraps <cds-accordion-item>                                 │
 * │   - Syncs props via modifier                                   │
 * └─────────────────────────────────────────────────────────────────┘
 *
 * CONTEXTUAL COMPONENT PATTERN:
 * The accordion yields an `Item` component that is pre-configured with
 * the accordion's size, type, and forceState. This allows consumers to
 * use the simple `<Acc.Item>` syntax while ensuring consistent styling.
 */
export default class HdsAccordion extends Component<HdsAccordionSignature> {
  ...
  /**
   * Create the Item component with inherited properties.
   *
   * This allows the parent Accordion to pass down configuration to all
   * child items automatically. The consumer can still override these
   * on individual items if needed.
   */
  get ItemComponent(): ComponentLike<HdsAccordionItemSignature> {
    // Note: In a real implementation, you might use the `component` helper
    // with curried arguments. For .gts, we return the component directly
    // and pass inherited args through the hash in the template.
    return HdsAccordionItem;
  }
