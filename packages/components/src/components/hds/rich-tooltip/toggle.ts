...
import type { ModifierXxxx } from '../popover-primitive/types.ts';
...
interface HdsRichTooltipToggleSignature {
  Args: {
    text?: string;
    icon?: string;
    ...
    isOpen?: boolean;
    popoverId: string;
    setupPrimitiveToggle: ModifierXxxx;
  };
  Blocks: {
    default: [];
  };
...
export default class HdsRichTooltipToggleComponent extends Component<HdsRichTooltipToggleSignature> {
  ...
  get size() {
    ...
    if (this.args.text) {
      size = this.args.size as string | undefined;
