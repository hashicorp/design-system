...
const OUTPUT_PATH = 'src/utils/carbon-import-map.ts'; // Adjust path if needed
const SIZES = ['16', '24'];

const hdsCarbonIconMap = {
...
// Extract unique Carbon names (values from the map)
const uniqueCarbonIcons = [...new Set(Object.values(hdsCarbonIconMap))];

// Generate Content
let content = `/**
 * Auto-generated file via generate-icons.js.
 * Do not edit manually.
 */
export const carbonMap: Record<string, () => Promise<any>> = {\n`;

uniqueCarbonIcons.sort().forEach((name) => {
  SIZES.forEach((size) => {
    // Key format: iconName/size
    content += `  '${name}/${size}': () => import('@carbon/icons/es/${name}/${size}.js'),\n`;
  });
});
...
console.log(`   - Unique Icons: ${uniqueCarbonIcons.length}`);
console.log(`   - Sizes: ${SIZES.join(', ')}`);
console.log(`   - Total Entries: ${uniqueCarbonIcons.length * SIZES.length}`);
