...
module('Integration | Component | hds/side-nav/index', function (hooks) {
  ...
  test('it renders the component', async function (assert) {
    await render(
      hbs`<Hds::SideNav::Section::Link @label="Terraform" @icon="terraform" @href="#" />`
    );
  ...
  test('it renders the passed in args', async function (assert) {
    await render(
      hbs`<Hds::SideNav::Section::Link @icon="boundary" @label="Boundary" @count="3" @badge="Alpha" @hasSubItems={{true}} />`
    );
  ...
  test('it renders the passed in custom content', async function (assert) {
    await render(hbs`
      <Hds::SideNav::Section::Link @label="Terraform" @hasSubItems={{true}}>
