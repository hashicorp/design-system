...
module('Integration | Component | hds/link-to/cta', function (hooks) {
  ...
  test('it renders a link(cta) with the defined text', async function (assert) {
    await render(hbs`<Hds::LinkTo::Cta @text="sign in" @route="index" />`);
  ...
  test('it should render with a CSS class that matches the component name and has the `button` class as well', async function (assert) {
    assert.expect(2);
    await render(
      hbs`<Hds::LinkTo::Cta @text="sign in" @route="index" id="test-link" />`
    );
  ...
  test('it should render the correct CSS size class if the @size prop is declared', async function (assert) {
    await render(
      hbs`<Hds::LinkTo::Cta @text="sign in" @route="index" id="test-link" @size="small" />`
    );
  ...
  test('it should render the icon in the leading position by default', async function (assert) {
    await render(
      hbs`<Hds::LinkTo::Cta @icon="arrow-right" @text="sign in" @route="index" id="test-link" />`
    );
  ...
  test('it should render the icon in the trailing position if @iconPosition is set to trailing', async function (assert) {
    await render(
      hbs`<Hds::LinkTo::Cta @icon="arrow-right" @text="sign in" @route="index" @iconPosition="trailing" id="test-link" />`
    );
  ...
  test('it should throw an assertion if @text is missing/has no value', async function (assert) {
    const errorMessage = '@text for "Hds::LinkTo::Cta" must have a valid value';
    assert.expect(2);
    setupOnerror(function (error) {
      assert.strictEqual(error.message, `Assertion Failed: ${errorMessage}`);
    });
    await render(hbs`<Hds::LinkTo::Cta @route="index" />`);
  ...
  test('it should throw an assertion if an incorrect value for @iconPosition is provided', async function (assert) {
    const errorMessage =
      '@iconPosition for "Hds::LinkTo::Cta" must be one of the following: leading, trailing; received: after';
    assert.expect(2);
    setupOnerror(function (error) {
      assert.strictEqual(error.message, `Assertion Failed: ${errorMessage}`);
    });
    await render(
      hbs`<Hds::LinkTo::Cta @icon="arrow-right" @iconPosition="after" @text="sign in" @route="index" />`
    );
    assert.throws(function () {
      throw new Error(errorMessage);
    });
  ...
  test('it should throw an assertion if an incorrect value for @size is provided', async function (assert) {
    const errorMessage =
      '@size for "Hds::LinkTo::Cta" must be one of the following: small, medium, large; received: tiny';
    assert.expect(2);
    setupOnerror(function (error) {
      assert.strictEqual(error.message, `Assertion Failed: ${errorMessage}`);
    });
    await render(
      hbs`<Hds::LinkTo::Cta @text="sign in" @size="tiny" @route="index" />`
    );
    assert.throws(function () {
      throw new Error(errorMessage);
    });
