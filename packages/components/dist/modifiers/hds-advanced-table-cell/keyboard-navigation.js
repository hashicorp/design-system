...
const handleGridCellKeyPress = (event, enableFocusTrap) => {
  ...
  const findNewRow = (currentRow, direction) => {
    ...
    const allVisibleRows = grid?.querySelectorAll('[role="row"]:not(.hds-advanced-table__tr--hidden)');
    if (allVisibleRows) {
      const currentRowIndex = Array.from(allVisibleRows).indexOf(currentRow);
      if (direction === 'ArrowDown') return allVisibleRows[currentRowIndex + 1];else if (direction === 'ArrowUp') return allVisibleRows[currentRowIndex - 1];
    }
  };
  if (target instanceof HTMLElement && (target.classList.contains('hds-advanced-table__th') || target.classList.contains('hds-advanced-table__td'))) {
    if (key === 'Enter') {
      ...
      if (event.ctrlKey) {
        ...
        const allVisibleRows = table?.querySelectorAll('[role="row"]:not(.hds-advanced-table__tr--hidden)');
        if (allVisibleRows) {
          const nextRow = key === 'Home' ? allVisibleRows[0] : allVisibleRows[allVisibleRows.length - 1];
          if (nextRow) {
      ...
      if (currentRow instanceof HTMLElement) {
        ...
        const allVisibleRows = grid?.querySelectorAll('[role="row"]:not(.hds-advanced-table__tr--hidden)');
        if (allVisibleRows) {
          const nextRow = event.key === 'PageUp' ? allVisibleRows[0] : allVisibleRows[allVisibleRows.length - 1];
          if (nextRow !== null && nextRow instanceof HTMLElement) {
            const nextCell = nextRow.children[currentCellIndex];
            if (nextCell instanceof HTMLElement) {
              changeActiveCell(target, nextCell);
