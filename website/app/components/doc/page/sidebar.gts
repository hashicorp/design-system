...
interface DocPageSidebarSignature {
  Args: {
    toc: {
      tree: TocTree;
    };
...
const getTocSectionsBundle = (section: string) => {
...
const getSidebarStructuredTree = (
  subTree: TocTree,
  filterQuery: string,
  currentURL: string,
) => {
...
const isActiveTree = (subTree: TocTree, currentURL?: string) => {
  let isActive = false;
  Object.keys(subTree).forEach((key) => {
    const item = subTree[key];
    if (item?.['pageURL']) {
...
export default class DocPageSidebar extends Component<DocPageSidebarSignature> {
  @service router!: Services['router'];
  private _currentTopRoute: string;

  @tracked filterQuery = '';

  constructor(owner: Owner, args: DocPageSidebarSignature['Args']) {
    super(owner, args);
  ...
  get structuredPageTree() {
    ...
    if (currentRoute.localName === 'show') {
      ...
      if (typeof currentRoute.params?.['path'] === 'string') {
    ...
    getTocSectionsBundle(currentSection).forEach((section) => {
      ...
      if (sectionTocTree) {
        ...
        const sidebarSubTree = getSidebarStructuredTree(
          sectionTocTree,
          this.filterQuery,
          currentRoute.params?.['path'] as string,
        );
  ...
  filterPageTree = task({ restartable: true }, async (filterQuery: string) => {
