...
In addition to the checked and unchecked states, a checkbox can be in an indeterminate state, also referred to as partially checked state. A common use case for this state is when a parent checkbox allows the user select multiple children checkboxes at once. The visual appearance of the checkbox is determined by the component based on the `indeterminate` attribute using JavaScript.
