...
To change this behavior, you can use the `@preserveDomContent` argument so that the list content is rendered in the DOM, regardless of whether the Dropdown is open or closed.

In this case, you can have a fine control of what is rendered and what is not using the yielded `[D].isOpen` state to conditionally render some of the content only when the Dropdown is open (eg. for lazy-loading some list items or content, while the rest is preloaded).
