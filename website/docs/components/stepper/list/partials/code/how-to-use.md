...
If you need to conditionally show the `<:description>` or `<:content>`, you must add `~` in your conditional blocks. The `<:description>` and `<:content>` are shown based on the CSS `:empty` selector which requires no whitespace be present.
