...
The following predefined segments can be passed into the Segmented Group as yielded contextual components: `Button`, `Dropdown`, `Select`, `TextInput`, `SuperSelect`, `SuperSelectMultiple`. For bespoke Segments use the `Generic` contextual component and style it accordingly.
